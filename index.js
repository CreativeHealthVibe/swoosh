...
app.get('/messages', (req, res) => {
  ...
  client.guilds.cache.forEach(guild => {
    const textChannels = guild.channels.cache
      .filter(channel => channel.type === 0) // 0 is GUILD_TEXT channel type
      .map(channel => ({
        id: channel.id,
        name: channel.name,
        permissions: channel.permissionsFor(guild.members.me)?.has('ManageWebhooks') || false
      }));
  ...
  const colorPresets = [
    { name: 'Default', value: config.embedColor },
  ...
  res.render('messages', {
    title: 'SWOOSH Bot - Message Manager',
    client: client,
    guilds: guilds,
...
// API endpoint for sending messages from the web interface
app.post('/api/send-message', async (req, res) => {
  try {
    // Validate required fields
    const { channelId, title, description } = req.body;
    const color = req.body.color || config.embedColor;
    const footer = req.body.footer || null;
    
    if (!channelId || !title || !description) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: channelId, title, and description are required'
      });
    }
    ...
    if (!permissions.has('ManageWebhooks')) {
      return res.status(403).json({
        success: false,
        error: 'Bot does not have permission to manage webhooks in this channel'
      });
    }
    ...
    try {
      ...
      if (validatedColor && !validatedColor.startsWith('#')) {
        ...
        validatedColor = colorMap[validatedColor.toLowerCase()] || config.embedColor;
      }
    } catch (colorError) {
      console.error('Color validation error:', colorError);
      validatedColor = config.embedColor;
    }
    ...
    if (footer) {
      ...
      embed.setFooter({
        text: `Sent via SWOOSH Web Dashboard`,
        iconURL: client.user.displayAvatarURL()
      });
    }
    
    // Send the message
    await webhookResult.webhook.send({
    ...
    // Log the action
    logging.logAction('Web Message Sent', null, null, {
    ...
    console.error('Error sending message from web interface:', error);
    res.status(500).json({
      success: false,
      error: error.message,
      timestamp: new Date().toISOString()
...
// API endpoint to get channels for a specific server
app.get('/api/channels', (req, res) => {
  try {
    ...
    // Get text channels where the bot has permission to manage webhooks
    const channels = guild.channels.cache
      .filter(channel => channel.type === 0) // Type 0 is GUILD_TEXT
...
// API endpoint to send an embed message
app.post('/api/send-embed', async (req, res) => {
  try {
    const { serverId, channelId, embed } = req.body;
    ...
    let embedColor = embed.color || config.embedColor;
    ...
    if (embedColor && !embedColor.startsWith('#')) {
      ...
      embedColor = namedColors[embedColor.toLowerCase()] || config.embedColor;
    }
    ...
    const messageEmbed = {
      ...
      footer: embed.footer ? { text: embed.footer } : { text: 'SWOOSH Bot' }
    };
    ...
    // Log the action
    logging.logAction('Embed Sent', { tag: 'Website User' }, null, {
      channel: channel.name,
      guild: guild.name,
    ...
    console.error('Send Embed API Error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to send embed'
    });
    
    logging.logAction('Send Embed Error', null, null, {
      error: error.message
    });
  }
});
