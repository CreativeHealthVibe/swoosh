  ...
  async execute(message, args, client) {
    ...
    try {
      ...
      try {
        ...
        // Continue execution even if DM fails - this is expected behavior for users with DMs disabled
      }
      
      // Record the warning in the Discord database
      const warningId = `${user.id}-${Date.now()}`;
      const warningData = {
        userId: user.id,
      ...
      // Add warning to Discord database
      ...
      const warnings = client.discordDB.findDocuments('warnings', (doc) => {
        return doc.userId === user.id && doc.guildId === message.guild.id;
      });
      ...
      message.channel.send('❌ An error occurred while processing the warning. The user may still have been warned, but there was a problem completing the operation.');
    }
  }
};
