...
module.exports = {
    ...
    async execute(message, args, client) {
        try {
            ...
            // Special handling for just a single time value followed by description
            if (args.length >= 2) {
                const firstArg = args[0];
                // If the first argument looks like a time value (e.g., "5m")
                if (firstArg.match(/^\d+[hms]$/)) {
                    console.log(`First arg is a time value: ${firstArg}`);
                    const timeValue = firstArg;
                    const description = args.slice(1).join(' ');
                    
                    // Process this single time value
                    const value = parseInt(timeValue.slice(0, -1));
                    const unit = timeValue.slice(-1).toLowerCase();
                    
                    if (isNaN(value) || value <= 0) {
                        return message.reply({
                            content: `❌ Invalid time value: \`${timeValue}\`. Please provide a positive number.`,
                            ephemeral: true
                        });
                    }
                    ...
                    if (unit === 'h') {
                        ...
                        formattedTime = `${value} hour${value !== 1 ? 's' : ''}`;
                    } else if (unit === 'm') {
                        totalMs = value * 60000; // minutes to ms
                        formattedTime = `${value} minute${value !== 1 ? 's' : ''}`;
                    } else if (unit === 's') {
                        totalMs = value * 1000; // seconds to ms
                        formattedTime = `${value} second${value !== 1 ? 's' : ''}`;
                    } else {
                        return message.reply({
            ...
            // Standard parsing for multiple time arguments
            const timeArgs = [];
